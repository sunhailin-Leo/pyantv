...
| 文件 | 操作 | 说明 |
|------|------|------|
| `pyantv/xxx/yyy.py` | 新增 | 实现 XxxClass |
| `test/test_xxx.py` | 新增 | XxxClass 的单元测试 |
| `pyantv/zzz.py` | 修改 | 添加 xxx 支持 |
...
- [ ] **M-BUILD**: `make build` 返回 0
- [ ] **M-REGRESSION**: `pytest -v --cov-config=.coveragerc --cov=./ test/` 全部 PASS
- [ ] **M-TEST**: 新增/修改的每个公开函数有测试，增量覆盖率 ≥ 70%
- [ ] **M-VERIFY**: （在此填写具体的 FAIL_TO_PASS 验证命令）
  - 实现前: `pytest test/test_xxx.py::test_new_feature` → FAIL
  - 实现后: `pytest test/test_xxx.py::test_new_feature` → PASS
- [ ] **M-RENDER**: （涉及渲染变更时填写）
  - 生成的 HTML 可正常打开
  - AntV G2 选项 JSON 结构正确
- [ ] （在此添加更多 MUST 条款）
