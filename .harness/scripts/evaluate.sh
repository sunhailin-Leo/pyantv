#!/usr/bin/env bash
# ============================================================
# pyantv — Sprint Evaluation Script
#
...
echo "============================================================"
echo "  pyantv — Sprint $SPRINT_NUM Evaluation"
...
if pytest -v --cov-config=.coveragerc --cov=./ test/ -q 2>/dev/null; then
...
COV_OUTPUT=$(pytest -v --cov-config=.coveragerc --cov=./ --cov-report=term test/ 2>&1) || true
...
RENDER_SCORE=0
RENDER_TESTS=$(pytest test/ -v -k "render" --co -q 2>/dev/null | wc -l | tr -d ' ')
...
| 维度 | 得分 | 满分 | 说明 |
|------|------|------|------|
| 构建正确性 | $SCORE_BUILD_ACTUAL | $SCORE_BUILD | make build |
